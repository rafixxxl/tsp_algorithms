...
int findFarthestPoint(vector<Point>& points, vector<bool>& visited, int current) {
...
vector<int> tspFarthestInsertion(vector<Point>& points) {
    int n = points.size();
    ...
    int farthest = findFarthestPoint(points, visited, current);
    path.push_back(farthest);
    visited[farthest] = true;

    int secondFarthest = findFarthestPoint(points, visited, current);
    path.push_back(secondFarthest);
    visited[secondFarthest] = true;

    current = secondFarthest;

    for (int i = 0; i < n - 3; i++) {
        int newPoint = findFarthestPoint(points, visited, current);
...
int main() {
    ...
    vector<int> path = tspFarthestInsertion(points);
