...
void apply3Opt(vector<Point>& points, vector<int>& path) {
    int n = points.size();
    bool improved = true;
            ...
            for (int j = i + 2; j < n - 1; j++) {
                for (int k = j + 2; k < n; k++) {
                    double distCurrent = distance(points[path[i]], points[path[i + 1]]) +
                                        distance(points[path[j]], points[path[j + 1]]) +
                                        distance(points[path[k]], points[path[k % n]]);
                    double distNew = distance(points[path[i]], points[path[j]]) +
                                     distance(points[path[i + 1]], points[path[k]]) +
                                     distance(points[path[j + 1]], points[path[(k + 1) % n]]);
